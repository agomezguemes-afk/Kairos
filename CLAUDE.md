...
- Mode: local-stdio
- Engine: postgres (Supabase, eu-west-1, project zduwnnbzvlijenghqegk)
- Pooler: Session pooler port 5432 (Transaction pooler 6543 not enabled)
- Config file: ~/.gbrain/config.json (mode 0600)
- Setup date: 2026-05-09
- MCP registered: yes (user scope, stdio)
- Artifacts sync: off (defer; re-run /setup-gbrain to enable)
- Transcript ingest: off (defer; re-run /setup-gbrain to enable)
- Current repo policy: read-write (github.com/agomezguemes-afk/kairos)
...
- This repo's code (registered as `gstack-code-kairos` source).
- `~/.gstack/` curated memory (registered as `gstack-brain-alvaro` source via
...
- "What did we decide last time?" / past plans, retros, learnings:
    `gbrain search "<terms>" --source gstack-brain-alvaro`
