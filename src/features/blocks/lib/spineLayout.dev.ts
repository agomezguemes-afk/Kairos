...
check(
  'out-of-order input → sorted by order field',
  sorted[0].id === b.id && sorted[1].id === c.id && sorted[2].id === a.id,
);

// ── column section + children ────────────────────────────────────────────
const sec = createColumnSectionNode(0, 2);
const child1: ContentNode = {
  ...createTextNode(1, 'paragraph', 'left'),
  section: sec.id,
  column: 0,
};
const child2: ContentNode = {
  ...createTextNode(2, 'paragraph', 'right'),
  section: sec.id,
  column: 1,
};
