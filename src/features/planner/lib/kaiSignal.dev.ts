...
check(
  'first-time',
  kaiSignal({ ...base, resolved: planned, lastSession: null })?.id === 'first-time',
);
check(
  'progress up',
  kaiSignal({ ...base, resolved: planned, lastSession: { setCount: 9, targetSetCount: 9 } })?.id ===
    'progress-up',
);

const close = kaiSignal({
  ...base,
  resolved: planned,
  lastSession: { setCount: 7, targetSetCount: 9 },
});
...
const close1 = kaiSignal({
  ...base,
  resolved: planned,
  lastSession: { setCount: 8, targetSetCount: 9 },
});
...
check(
  'past completed silent',
  kaiSignal({ ...base, isToday: false, isPast: true, resolved: completed }) === null,
);
