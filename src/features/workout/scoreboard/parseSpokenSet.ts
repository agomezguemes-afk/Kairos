...
//
// Digits AND spelled-out Spanish numbers coexist: "sesenta y dos por ocho"
// parses identically to "62 por 8" thanks to spanishNumbers.ts, folded into
// number tokens right after tokenize() so the whole pipeline sees plain numbers.
// Effort phrases ("me costó") are NOT converted to RPE (guessing is noise); only
// the explicit "rpe N" pattern sets it.
...
// Spanish spelled-out numbers → number tokens, BEFORE steps 1-6, so "sesenta
// por ocho" walks the exact same pipeline as "60 por 8". Only maximal runs of
// consecutive text tokens are scanned; a run that doesn't start a number is
// left untouched (e.g. "por", "rpe", "kg" pass through as text).
function foldSpanishNumbers(tokens: Token[]): Token[] {
  ...
  for (let i = 0; i < tokens.length; ) {
    if (tokens[i].kind === 'text') {
      ...
      if (run) {
        out.push({ raw: String(run.value), kind: 'number', consumed: false });
        i += run.length;
        continue;
      }
    }
...
function parse(utterance: string, fields: FieldDefinition[]): ParsedSet {
  ...
  const normalized = utterance
    ...
    .trim()
    .replace(/(\d),(\d)/g, '$1.$2');
  const tokens = foldSpanishNumbers(tokenize(normalized));
  ...
  const resolveUnitField = (dest: UnitDest): FieldDefinition | null => {
    ...
    if (dest.fieldId) {
      const byId = numericFields.find(
        (f) => f.id === dest.fieldId && f.type === 'number' && free(f),
      );
      if (byId) return byId;
    }
    if (dest.unit) {
      const byUnit = numericFields.find(
        (f) => f.unit === dest.unit && f.type === 'number' && free(f),
      );
