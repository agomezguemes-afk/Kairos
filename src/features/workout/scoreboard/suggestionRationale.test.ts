...
import { describeSuggestion, describeInSessionNudge } from './suggestionRationale';
...
describe('describeInSessionNudge', () => {
  it('easy: "+2.5 kg" and "fácil" — la serie anterior, not la última', () => {
    const c = describeInSessionNudge({ deltaKg: 2.5, nextWeight: 62.5, reason: 'easy' });
    expect(c.text).toContain('+2.5 kg');
    expect(c.text).toContain('fácil');
    expect(c.text).toContain('serie anterior');
  });

  it('hard: "−2.5 kg" (U+2212, no ASCII hyphen) and "costó"', () => {
    const c = describeInSessionNudge({ deltaKg: -2.5, nextWeight: 57.5, reason: 'hard' });
    expect(c.text).toContain(`${MINUS}2.5 kg`);
    expect(c.text).not.toContain('-'); // no ASCII hyphen
    expect(c.text).toContain('costó');
  });

  it('both reasons produce non-empty spoken lines', () => {
    const easy = describeInSessionNudge({ deltaKg: 2.5, nextWeight: 62.5, reason: 'easy' });
    const hard = describeInSessionNudge({ deltaKg: -2.5, nextWeight: 57.5, reason: 'hard' });
    expect(easy.spoken).toBeTruthy();
    expect(easy.spoken).toContain('fácil');
    expect(hard.spoken).toBeTruthy();
    expect(hard.spoken).toContain('costó');
  });
});
