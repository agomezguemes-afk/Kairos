...
check(
  'block-a present on may 12',
  (may.get('2026-05-12') ?? []).some((r) => r.blockId === 'block-a'),
);
...
check(
  'skip removes occurrence',
  may4.every((r) => r.assignmentId !== id2),
);
...
check(
  'move clears origin',
  may6.every((r) => r.assignmentId !== id2),
);
check(
  'move populates target',
  may7.some((r) => r.assignmentId === id2 && r.movedFrom === '2026-05-06'),
);
...
check(
  'completed status',
  completed.some((r) => r.assignmentId === id1 && r.status === 'completed'),
);
...
const stillRecurring = Array.from(lateMay.values())
  .flat()
  .filter((r) => r.assignmentId === id2);
