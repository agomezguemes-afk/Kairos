...
check(
  'all have muscleGroups',
  EXERCISE_LIBRARY.every((e) => e.muscleGroups.length >= 1),
);
...
check(
  'all templates ≥3 exercises',
  BLOCK_TEMPLATES.every((t) => t.exercises.length >= 3),
);
check(
  'all template refs valid',
  BLOCK_TEMPLATES.every((t) => t.exercises.every((te) => getLibraryEntry(te.libraryId) !== null)),
);
...
check(
  'exercises link block',
  exercises.every((e) => e.workout_block_id === block.id),
);
