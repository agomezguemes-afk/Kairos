...
const emptyBlock: WorkoutBlock = {
  id: 'b1', user_id: 'u', name: 'Test', icon: '', color: '#000',
  description: null, tags: [], discipline: 'strength',
  content: [], layout: { columns: 1 }, status: 'draft', is_favorite: false,
  is_archived: false, last_performed_at: null, times_performed: 0,
  sort_order: 0, size: 'medium', cover: null,
...
const benchHistory: WorkoutHistoryEntry[] = [
  ...
  {
    id: 'old', blockId: 'b1', blockName: 'Push',
    startedAt: nowMs - 35 * day, endedAt: nowMs - 35 * day + 3600_000,
    exerciseCount: 1, setCount: 3, totalVolume: 1200, durationSec: 3600,
    exercises: [{
      exerciseId: 'ex_old', libraryId: 'bench_press', name: 'Bench Press',
      maxWeight: 70, totalVolume: 70 * 24, setsCompleted: 3,
      performedSets: [
  ...
  {
    id: 's1', blockId: 'b1', blockName: 'Push',
    startedAt: nowMs - 21 * day, endedAt: nowMs - 21 * day + 3600_000,
  ...
  {
    id: 's2', blockId: 'b1', blockName: 'Push',
    startedAt: nowMs - 7 * day, endedAt: nowMs - 7 * day + 3600_000,
...
const unbound = computeDashboardValue(
  ...
  emptyBlock, benchHistory, nowMs,
);
check('unbound exercise dashboard is empty', unbound.isEmpty);
check('unbound caption prompts selection',
  unbound.caption === 'Selecciona un ejercicio');

// ── Exercise-scope: exercise_max_weight, lookback 4w ───────────────────────
const baseData: DashboardNodeData = {
  metric: 'exercise_max_weight', viz: 'counter', label: 'Peso máximo',
  color: '#C9A96E', libraryId: 'bench_press', exerciseName: 'Bench Press',
  lookback: '4w',
};
...
check('max_weight 4w shows all-time-record caption (85 is best in window AND ever)',
  maxW.caption?.includes('Récord absoluto') ?? false);

// Same exercise, lookback 'all' — old 70 included, but max still 85.
const maxAll = computeDashboardValue({ ...baseData, lookback: 'all' }, emptyBlock, benchHistory, nowMs);
...
const maxSession = computeDashboardValue(
  { ...baseData, lookback: 'session' }, emptyBlock, benchHistory, nowMs,
);
...
const vol4w = computeDashboardValue(
  { ...baseData, metric: 'exercise_volume' }, emptyBlock, benchHistory, nowMs,
);
...
const rm4w = computeDashboardValue(
  { ...baseData, metric: 'exercise_estimated_1rm' }, emptyBlock, benchHistory, nowMs,
);
...
const freq4w = computeDashboardValue(
  { ...baseData, metric: 'exercise_freq' }, emptyBlock, benchHistory, nowMs,
);
...
const lastTop = computeDashboardValue(
  { ...baseData, metric: 'exercise_last_top' }, emptyBlock, benchHistory, nowMs,
);
...
check('no history caption is "Sin sesiones registradas"',
...
const customHist: WorkoutHistoryEntry[] = [{
  id: 'c1', blockId: 'b1', blockName: 'Custom',
  startedAt: nowMs - 3 * day, endedAt: nowMs - 3 * day + 3600_000,
  exerciseCount: 1, setCount: 1, totalVolume: 100, durationSec: 3600,
  exercises: [{
    exerciseId: 'custom', name: 'My Weird Pull',
    maxWeight: 50, totalVolume: 50 * 6, setsCompleted: 1,
    performedSets: [{ weight: 50, reps: 6, completed: true }],
  }],
}];
const byName = computeDashboardValue(
  { metric: 'exercise_max_weight', viz: 'counter', label: '', color: '#000',
    exerciseName: 'My Weird Pull', lookback: '4w' },
  emptyBlock, customHist, nowMs,
);
