...
check('normalize strips diacritics',    normalizeExerciseName('Sentadilla búlgara') === 'sentadilla bulgara');

// ── isSameExercise ─────────────────────────────────────────────────────────
check('same libraryId matches',
  isSameExercise({ libraryId: 'bench_press', name: 'Bench' }, { libraryId: 'bench_press', name: 'Press' }));
check('different libraryId does not match',
  !isSameExercise({ libraryId: 'a', name: 'X' }, { libraryId: 'b', name: 'X' }));
check('same normalized name matches (no libraryId)',
  isSameExercise({ name: 'Bench Press' }, { name: 'bench press' }));
check('libraryId beats name match',
...
const history: WorkoutHistoryEntry[] = [
  {
    id: 's1', blockId: 'b1', blockName: 'Push A', startedAt: tBase, endedAt: tBase + 3600_000,
    exerciseCount: 1, setCount: 3, totalVolume: 1800, durationSec: 3600,
    exercises: [
      {
        exerciseId: 'ex_bench_1', libraryId: 'bench_press', name: 'Bench Press',
        maxWeight: 80, totalVolume: 80 * 24, setsCompleted: 3,
        performedSets: [
  ...
  {
    id: 's2', blockId: 'b1', blockName: 'Push A', startedAt: tBase + 3 * day, endedAt: tBase + 3 * day + 3600_000,
    exerciseCount: 2, setCount: 5, totalVolume: 3200, durationSec: 3600,
    exercises: [
      {
        exerciseId: 'ex_bench_2', libraryId: 'bench_press', name: 'Bench Press',
        maxWeight: 82.5, totalVolume: 82.5 * 24, setsCompleted: 3,
        performedSets: [
      ...
      {
        exerciseId: 'ex_squat_1', libraryId: 'squat', name: 'Back Squat',
        maxWeight: 100, totalVolume: 100 * 20, setsCompleted: 2,
        performedSets: [
  ...
  {
    id: 's3', blockId: 'b1', blockName: 'Push A', startedAt: tBase + 7 * day, endedAt: tBase + 7 * day + 3600_000,
    exerciseCount: 1, setCount: 3, totalVolume: 2000, durationSec: 3600,
    exercises: [
      {
        // Same exercise but logged from a custom (no-library) card — name fallback.
        exerciseId: 'ex_bench_3', name: 'Bench Press',
        maxWeight: 85, totalVolume: 85 * 24, setsCompleted: 3,
        performedSets: [
...
// ── getExerciseHistoryFor ──────────────────────────────────────────────────
const benchHistory = getExerciseHistoryFor({ libraryId: 'bench_press', name: 'Bench Press' }, history);
check('bench history has 3 sessions',   benchHistory.length === 3);
check('bench history asc-sorted',       benchHistory[0].at < benchHistory[1].at && benchHistory[1].at < benchHistory[2].at);
...
check('bench history limit=2',          getExerciseHistoryFor({ name: 'Bench Press' }, history, 2).length === 2);
...
const longHistory: ExerciseSessionPoint[] = Array.from({ length: 12 }, (_, i) => ({
  at: tBase + i * day, date: '2026-05-01', topWeight: 70 + i, topReps: 5, volume: 100,
  setsCompleted: 1, estimatedOneRm: 80 + i,
}));
...
const candidateWeightPr: ExerciseSessionPoint = {
  at: tBase + 10 * day, date: '2026-05-11', topWeight: 90, topReps: 6, volume: 90 * 24,
  setsCompleted: 3, estimatedOneRm: estimateOneRepMax(90, 6),
};
...
const noChange: ExerciseSessionPoint = {
  at: tBase + 11 * day, date: '2026-05-12', topWeight: 80, topReps: 5, volume: 1000,
  setsCompleted: 3, estimatedOneRm: estimateOneRepMax(80, 5),
};
...
const firstEver: ExerciseSessionPoint = {
  at: tBase, date: '2026-05-01', topWeight: 60, topReps: 8, volume: 60 * 24,
  setsCompleted: 3, estimatedOneRm: estimateOneRepMax(60, 8),
};
...
const cheatPr: ExerciseSessionPoint = {
  at: tBase + 12 * day, date: '2026-05-13', topWeight: 100, topReps: 1, volume: 100,
  setsCompleted: 1, estimatedOneRm: estimateOneRepMax(100, 1),
};
const cheatResult = detectPr(cheatPr, benchHistory);
check('100×1 vs 85×6 is not a weight PR', cheatResult.kind !== 'weight');

// ── getLastCompletedReference ──────────────────────────────────────────────
const lastRef = getLastCompletedReference({ libraryId: 'bench_press', name: 'Bench Press' }, history);
